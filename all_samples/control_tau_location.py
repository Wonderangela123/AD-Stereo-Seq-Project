...
## Extract neurons in control samples
...
id_control = merged_data.tl.result['diagnosis']['group'].str.contains('control')
id_control = id_control[id_control].index.tolist()
id_neuron_control= list(set(id_neuron).intersection(set(id_control)))
data1 = merged_data
data1.cells.cell_name = merged_data.cells.cell_name[id_neuron_control]
data1.exp_matrix = merged_data.exp_matrix[id_neuron_control]
data1.position = merged_data.position[id_neuron_control, :]
...
## Draw plots
# six controls
for i in samples[6:11]:
    # Extract cell names of tau(case) from data1
    ...
    fig.savefig(os.path.join(path_dir, "control_{}_tau_location.png".format(samples[i])))
